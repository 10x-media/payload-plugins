...
export COMPOSE_PROJECT_NAME="form-builder_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/form-builder build

echo "Building dev app..."
pnpm --filter @10x-media/form-builder-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/form-builder exec playwright test "$@"

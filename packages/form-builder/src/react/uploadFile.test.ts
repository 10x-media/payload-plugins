...
describe('uploadFile', () => {
	it('POSTs the file to the given collection and returns the id', async () => {
		const fetchImpl = vi.fn().mockResolvedValue(okResponse({ doc: { id: 'up1' } }))
		const result = await uploadFile({ file: file(), collection: 'app-uploads', fetchImpl })
	...
	it('returns an error result on a non-ok response', async () => {
		...
		expect(await uploadFile({ file: file(), collection: 'media', fetchImpl })).toEqual({
			ok: false,
	...
	it('returns an error result when no id is returned', async () => {
		...
		const result = await uploadFile({ file: file(), collection: 'media', fetchImpl })

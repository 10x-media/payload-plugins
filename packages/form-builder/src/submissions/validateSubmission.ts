...
import { resolveSubmissionLocale, SUBMISSION_LOCALE_CONTEXT_KEY } from './submissionLocale'
...
export const validateSubmission =
	...
	async ({ data, operation, originalDoc, req }) => {
		...
		// `req.locale` comes from the visitor's `?locale=` (or `createSubmission`'s `locale`). Clamp it
		// before anything reads it (the form load below, the host's consent and calc resolvers) so a
		// visitor cannot run the pipeline under `all` or an unconfigured code. On a localized host the
		// clamped value is set on `req` only while this hook runs and restored after, because `req` may
		// be the host's own request; without localization Payload leaves its `locale` unset, and so do we.
		const { localization } = req.payload.config
		const requested = req.context[SUBMISSION_LOCALE_CONTEXT_KEY] ?? req.locale
		delete req.context[SUBMISSION_LOCALE_CONTEXT_KEY]
		const locale = resolveSubmissionLocale(requested, localization)

...
export type ResolveConsentStatementsArgs = {
	payload: Payload
	req: PayloadRequest
	/** A loaded forms document; its consent fields' `source` keys drive resolution. */
	form: {
		id: number | string
		title?: string | null
 ...
 *
 * The statement is a live reference, not a copy: a form stores only its source key, and the
...
export const resolveConsentStatements = async (
	...
	for (const field of consentFields) {
		const entry = entries.find((candidate) => candidate.key === field.source)
		if (!entry) {
			continue
		}
		// The link needs a name of its own, so it takes the source's `label` and never falls back to
		// the `key`: the key is a machine identifier, and "privacy" as the words a visitor clicks is
		// worse than showing no link at all. A source with a url and no label simply has no link.
		const label = typeof entry.label === 'string' && entry.label.trim() !== '' ? entry.label : ''

...
export function RelationshipContent(props: Props) {
	...
	function generateMetaText(mimeType: string, size: number): string {
		...
		if (size) sections.push(formatFilesize(size))
		if (x && y) sections.push(`${x}x${y}`)
		if (mimeType) sections.push(mimeType)
			...
			<div className={`${baseClass}__imageAndDetails`}>
				{previewAllowed && (
					<Thumbnail
						...
						imageCacheTag={collectionConfig?.upload?.cacheTags ? updatedAt : undefined}
						size="small"
					/>
				)}

...
export COMPOSE_PROJECT_NAME="impersonation_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter '@10x-media/impersonation-dev^...' build

echo "Building dev app..."
pnpm --filter @10x-media/impersonation-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/impersonation exec playwright test "$@"

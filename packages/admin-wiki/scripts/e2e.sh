...
export COMPOSE_PROJECT_NAME="admin-wiki_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/admin-wiki build

echo "Building dev app..."
pnpm --filter @10x-media/admin-wiki-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/admin-wiki exec playwright test "$@"

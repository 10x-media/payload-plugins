...
- Add `phoneNumberField()`: an international phone number field with a searchable country picker, live as-you-type formatting, and validation and metadata sized to the install via `libphonenumber-js`.
  ...
  - Country handling: `countries` scopes the picker without restricting what parses, validates, and saves; `priorityCountries` pins a subset above the alphabetized rest, optionally headed by `priorityCountriesLabel` (a string or per-locale map; no label means no heading, only the picker's own group divider); `defaultCountry` seeds the picker and calling-code prefix.
  - Flag artwork: `flags: 'svg'` (default) serves real country flags from a new, unauthenticated `/10x-fields/flags/:code` endpoint; `'emoji'` draws a Unicode regional-indicator pair instead (renders as two separate letters on Windows, which ships no flag emoji font); `'none'` shows the ISO code. `fields({ phoneNumber: { serveFlags: false } })` skips mounting the endpoint. `@10x-media/fields/phone` exports `countryFlagSrc(serverURL, apiRoute, code)` so a frontend can build the same URL without hand-rolling its lowercasing and `.svg` suffix.
  - `fields({ phoneNumber: {...} })` sets install-wide defaults (`defaultCountry`, `countries`, `priorityCountries`, `priorityCountriesLabel`, `validation`, `flags`, `cellFormat`, `metadata`, `serveFlags`); a field's own option always wins.
  - `@10x-media/fields/phone/utils` exports the parsing, formatting, and validation engine standalone for frontend use (`parsePhone`, `formatPhone`, `phoneUri`, `detectCountry`, `salvagePhone`, `checkPhone`, `formatAsYouType`, `countryOptions`, `callingCodeFor`, `emojiFlag`, `isKnownCountry`, `isSupported`, `loadMetadata`).

...
export COMPOSE_PROJECT_NAME="fields_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/fields build

echo "Building dev app..."
pnpm --filter @10x-media/fields-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/fields exec playwright test "$@"

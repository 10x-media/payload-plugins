...
/**
 * Reimplemented Payload eye glyph (`@payloadcms/ui` does not export one). `active`
 * true draws the struck-through eye (value shown, click to hide); false draws the
 * open eye (value concealed, click to reveal). Sized by the host button's
 * `--btn-icon-size`; only `.stroke` needs styling.
 */
const EyeGlyph: React.FC<{ active: boolean }> = ({ active }) => (
	<svg
		aria-hidden="true"
...
/**
 * The reveal toggle shared by every concealed field. Present in both masked and
 * revealed states so toggling never removes the focused control from the tree.
 */
export const EyeToggle: React.FC<{
	className?: string
	onToggle: () => void
	revealed: boolean
}> = ({ className, onToggle, revealed }) => {
	const { t } = useTranslation()
	const label = revealed ? t(keys.hideValue) : t(keys.showValue)
	return (
		<Button
			aria-label={label}
			buttonStyle="none"
			className={['tenx-protected-field__eye', className].filter(Boolean).join(' ')}
			extraButtonProps={{ 'aria-pressed': revealed }}
			icon={<EyeGlyph active={revealed} />}
			margin={false}
			onClick={onToggle}
		/>
	)

...
export COMPOSE_PROJECT_NAME="settings-overlay_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/settings-overlay build

echo "Building dev app..."
pnpm --filter @10x-media/settings-overlay-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/settings-overlay exec playwright test "$@"

...
export COMPOSE_PROJECT_NAME="sse_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/sse build

echo "Building dev app..."
pnpm --filter @10x-media/sse-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/sse exec playwright test "$@"

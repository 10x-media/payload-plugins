...
describe('analyticsProxyRewrites', () => {
	...
	it('targets a self-hosted Plausible instance and custom path', () => {
		const rewrites = analyticsProxyRewrites('plausible', {
			path: '/stats',
	...
	it('proxies the Umami script and send endpoint, defaulting host and path', () => {
		expect(analyticsProxyRewrites('umami')).toEqual([
			{ source: '/um/script.js', destination: 'https://cloud.umami.is/script.js' },
			{ source: '/um/api/send', destination: 'https://cloud.umami.is/api/send' },
		])
	})

	it('normalizes the proxy path', () => {
		expect(analyticsProxyRewrites('umami', { path: 'track/' })[0]?.source).toBe('/track/script.js')
	})
})

...
- **Dashboard widgets** for Payload's Modular Dashboard: metric, trend (with an optional previous-period overlay), goals, ten breakdowns, and realtime, each capability-gated, each linking into the analytics view, plus a public API for custom widgets.

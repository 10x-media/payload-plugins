...
export COMPOSE_PROJECT_NAME="wildix_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/wildix build

echo "Building dev app..."
pnpm --filter @10x-media/wildix-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/wildix exec playwright test "$@"

...
export COMPOSE_PROJECT_NAME="dual-session_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/dual-session build

echo "Building dev app..."
pnpm --filter @10x-media/dual-session-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/dual-session exec playwright test "$@"

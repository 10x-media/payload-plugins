 ...
 * and refusing to boot over it would be worse than saying so and moving on. Listing the
 * admin collection stays fatal: it is the one mistake that silently breaks the admin panel.
 *
...
export const selectCollections = ({
	...
	if (resolved.some(({ slug }) => slug === adminUserSlug)) {
		throw new Error(

...
describe('outbound delivery (inline)', () => {
	...
	it('delivers a signed POST and records success', async () => {
		...
		expect(hits[0]?.headers['webhook-signature']).toMatch(/^v1,[A-Za-z0-9+/]+={0,2}$/)
		expect(hits[0]?.headers['webhook-id']).toBeTruthy()
		expect(hits[0]?.headers['webhook-timestamp']).toMatch(/^\d+$/)

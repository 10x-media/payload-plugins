...
export function LogRow({ adminRoute, doc, userTitleFields }: Props) {
  ...
  const isExpandable = hasDiff || hasSnapshot || hasMetadata || isAuthOrCustom || hasMeta || hasGroup
        ...
        <span className="al-row__right">
          {pathCount > 0 && (
            <span className="al-row__paths-count">
              {t(pathCount !== 1 ? 'auditPlugin:fieldsChangedPlural' : 'auditPlugin:fieldsChanged', { count: pathCount })}
            </span>
          )}
            ...
            <div className="al-row__meta">
              {doc.ipAddress && (
                <span className="al-row__meta-item">
                  <span className="al-row__meta-label">{t('auditPlugin:metaIp')}</span> {doc.ipAddress}
                </span>
              )}
              {doc.userAgent && (
              ...
              {doc.locale && (
                <span className="al-row__meta-item">
                  <span className="al-row__meta-label">{t('auditPlugin:metaLocale')}</span> {doc.locale}
                </span>
              )}
              {doc.group && groupHref && (
                <span className="al-row__meta-item">
                  <span className="al-row__meta-label">{t('auditPlugin:filterGroup')}</span>{' '}
                  <a className="al-row__meta-group" href={groupHref}>{doc.group}</a>
                </span>
              )}
            </div>
            ...
            {isAuthOrCustom && (
              <div className="al-row__section">
                <div className="al-row__section-label">
                  {doc.operation === 'auth' ? t('auditPlugin:sectionAuthEvent') : t('auditPlugin:sectionCustomEvent')}
                </div>

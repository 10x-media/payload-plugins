...
const applyAnonymization = (
  ...
  for (const [path, { before, after }] of Object.entries(diff)) {
    result[path] = {
      before: anonymize({ path, value: before, collection: globalSlug, documentId: '', operation: 'update', redacted: REDACTED }),
      after: anonymize({ path, value: after, collection: globalSlug, documentId: '', operation: 'update', redacted: REDACTED }),
    }
  }

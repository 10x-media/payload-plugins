...
const applyAnonymization = (
  ...
  for (const [path, { before, after }] of Object.entries(diff)) {
    result[path] = {
      before: anonymize({ path, value: before, collection: collectionSlug, documentId, operation, redacted: REDACTED }),
      after: anonymize({ path, value: after, collection: collectionSlug, documentId, operation, redacted: REDACTED }),
    }
  }
...
export const afterChangeCollectionAuditLog =
  ...
  async ({ req, doc, previousDoc, operation }) => {
    ...
    if (options.drafts === 'ignore' && doc._status === 'published' && previousDoc?._status === 'draft') {
    ...
    if (operation === 'create') {
      ...
      const snapshotRaw = options.snapshotOnCreate
        ? options.anonymize
          ? anonymizeDoc(doc as Record<string, unknown>, options.collectionSlug, documentId, 'create', options.anonymize)
          : (doc as Record<string, unknown>)
        : undefined
      const snapshot = snapshotRaw && options.fieldMap

...
export type ArchiveTaskOptions = {
  ...
  populateUploadFields?: (args: { filename: string; logCount: number; runDate: Date }) => Record<string, unknown>
...
export const buildArchiveTask = (options: ArchiveTaskOptions): TaskConfig => ({
  ...
  handler: async ({ req, job }) => {
    ...
    const auditLogsCollection = req.payload.config.collections.find(
      (c) => c.slug === 'audit-logs',
    )

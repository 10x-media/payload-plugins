 ...
 */
export type DiffPaths<T, Prefix extends string = '', D extends number = 0> = D extends 8
	? never
	: {
			[K in keyof T & string]-?: IsRelationship<T[K]> extends true
				? `${Prefix}${K}`
 ...
 */
export type DiffPathValue<T, P extends string> = P extends `${infer K}.${infer Rest}`
	? K extends keyof T
		? NonNullable<T[K]> extends Array<infer Item>
			? Item extends { id: string | number }

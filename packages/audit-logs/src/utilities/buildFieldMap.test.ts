...
const group = (name: string, fields: Field[]): Field =>
  ({ type: 'group', name, fields }) as Field

const unnamedGroup = (fields: Field[]): Field =>
  ({ type: 'group', fields }) as Field
...
const array = (name: string, fields: Field[]): Field =>
  ({ type: 'array', name, fields }) as Field
...
describe('buildFieldMap', () => {
  ...
  describe('tabs', () => {
    ...
    it('mixed tabs: named and unnamed in the same field', () => {
      const map = buildFieldMap([
        tabs([
          namedTab('general', [rel('owner')]),
          unnamedTab([rel('reviewer')]),
        ]),
      ])
      expect(map.get('general.owner')).toBe('rel-single')
      expect(map.get('reviewer')).toBe('rel-single')
  ...
  describe('nesting', () => {
    ...
    it('relationship inside named tab inside array', () => {
      const map = buildFieldMap([
        array('steps', [
          tabs([namedTab('details', [rel('assignee')])]),
        ]),
      ])
    ...
    it('relationship inside row inside named group', () => {
      const map = buildFieldMap([
        group('settings', [
          row([rel('owner'), rel('contributors', true)]),
        ]),
      ])
      expect(map.get('settings.owner')).toBe('rel-single')
  ...
  describe('join fields', () => {
    it('records a join field as join kind', () => {
      const map = buildFieldMap([{ type: 'join', name: 'relatedPosts', collection: 'posts', on: 'author' } as Field])
      expect(map.get('relatedPosts')).toBe('join')
    })

    it('records a join field inside a named group', () => {
      const map = buildFieldMap([group('meta', [{ type: 'join', name: 'linked', collection: 'posts', on: 'ref' } as Field])])
      expect(map.get('meta.linked')).toBe('join')
    })

    it('records a join field inside an array (wildcard)', () => {
      const map = buildFieldMap([array('steps', [{ type: 'join', name: 'related', collection: 'posts', on: 'step' } as Field])])

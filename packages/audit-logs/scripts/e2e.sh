...
export COMPOSE_PROJECT_NAME="audit-logs_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/audit-logs build

echo "Building dev app..."
pnpm --filter @10x-media/audit-logs-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/audit-logs exec playwright test "$@"

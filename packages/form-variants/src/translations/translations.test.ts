...
describe('translations', () => {
	it('nests every flat key under the formVariants namespace', () => {
		const nested = translations.en.formVariants
		expect(nested, 'formVariants namespace missing from translations').toBeDefined()
		if (!nested) return
		for (const fullKey of Object.values(keys)) {
			const short = fullKey.slice('formVariants:'.length)
	...
	it('ships every locale nested under the plugin namespace', () => {
		for (const name of Object.keys(locales)) {
			expect(translations[name as keyof typeof translations], name).toHaveProperty('formVariants')
		}
	})
})

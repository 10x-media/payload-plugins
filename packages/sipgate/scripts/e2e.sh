...
export COMPOSE_PROJECT_NAME="sipgate_e2e_$(pwd | cksum | cut -d' ' -f1)"
...
pnpm --filter @10x-media/sipgate build

echo "Building dev app..."
pnpm --filter @10x-media/sipgate-dev build

echo "Running Playwright e2e..."
pnpm --filter @10x-media/sipgate exec playwright test "$@"

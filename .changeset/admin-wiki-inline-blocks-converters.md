...
- `wikiFeatures` is exported for a block that holds its own rich text field: `wikiFeatures({ ...wikiEditorOptions, nested: true })`. Arguments left out fall back to defaults.
- `nested` drops the callout, headings, lists, blockquotes, horizontal rules, indentation and the fixed toolbar.

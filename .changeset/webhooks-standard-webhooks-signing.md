...
- **Customer-supplied secrets**: accepted on create and on rotation, normalized to exactly one `whsec_` prefix, and rejected when they are not canonical padded base64 carrying at least 16 bytes. A ragged length is rejected rather than decoded down to a shorter key, so a clipped paste cannot silently become a different secret. Malformed code-subscription secrets now fail at startup rather than at delivery.

...
- **Scoping**: `operations`, `excludeFields`, `drafts: 'ignore'`, and a per-event `shouldLog` predicate that runs after the diff.
...
- **Auth events**: logins and password resets on every auth collection, independent of the `collections` map, controlled by `auth`.

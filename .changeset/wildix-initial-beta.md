...
- **Sync**: colleagues, devices, and call queues from the Wildix PBX API into `wildix-users`, `wildix-devices`, and `wildix-channels` collections, plus call history from the WDA analytics API into `call-logs`. Supports a shared API-key mode and per-user OAuth2.

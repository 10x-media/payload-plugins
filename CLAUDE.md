...
**Toolchain:** Node 22+ and pnpm 10 for development (published plugins still declare Payload's runtime range, `^18.20.2 || >=20.9.0`). Docker is required for the Postgres, container, and e2e test tiers. Shared dependency versions are centralized in a pnpm `catalog:` in `pnpm-workspace.yaml`, so a Payload or React bump is a one-line change. Cacheable tasks (build, lint, typecheck, test) run through Turborepo.

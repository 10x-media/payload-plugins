...
**Auth plugins in general.** Anything that writes `generatePayloadCookie` from code you do not control — most `payload-oauth2`-shaped plugins — bypasses isolation. If the plugin exposes a hook for setting the cookie, use it with `generateIsolatedAuthCookie`; if it does not, that collection cannot be isolated.

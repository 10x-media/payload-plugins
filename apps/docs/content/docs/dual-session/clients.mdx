...
This is also why an app-only Payload does not need the plugin: with no cookie in play there is nothing to isolate. A [role-split](/dual-session/role-split) collection changes nothing here either — both halves log in through the same route and get the same kind of token.

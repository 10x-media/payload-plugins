...
| `POST /refresh-token` | `req.user` | writes the cookie that user belongs in **now** |
| `POST /logout` | `req.user` | expires only that cookie |
| `GET /me` | `req.user` | reports that cookie's token |
| `POST /first-register` | Nobody | always the shared cookie |
...
Without `user` the call throws rather than pick a half. Same for `resolveIsolatedCookieName({ collection, payload, user })`. For a collection listed without a predicate, `user` stays optional, because there is only one answer. See [Custom auth](/dual-session/custom-auth).

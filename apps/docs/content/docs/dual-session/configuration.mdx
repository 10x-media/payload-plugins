...
| `scopes` | `['frontend']` | Which [scopes](/dual-session/scopes) this cookie may authenticate. Enforced on any request that carries the scope header, whoever set it; requests without one fall back to `adminSessionPriority`. |
...
Per-locale overrides keyed by the typed keys from `@10x-media/dual-session/i18n`. The plugin ships one string, `dualSession:pluginName`, in several locales, and renders no admin UI of its own.

...
Per-locale overrides keyed by the typed keys from `@10x-media/dual-session/i18n`. The plugin ships one string, `dualSession:pluginName`, in ten locales, and renders no admin UI of its own.

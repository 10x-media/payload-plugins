...
| --- | --- |
| Only reads `req.user` — access control, audit trails, field UI | **Fine.** Nothing here changes what `req.user` is once it has resolved. |
...
1. `generatePayloadCookie` — writes the shared cookie, conflict.
2. `path: '/login'` (or `/logout`, `/refresh-token`, `/me`) on a collection you isolate — endpoint conflict.
3. `parseCookies` or `extractJWT` used to authorize — reads the shared cookie, conflict.
4. `auth.strategies = [...]` rather than appending — drops this plugin's strategy.
...
On a collection [split by role](/dual-session/role-split), promoting a member to editor does not move their live session: it stays in the isolated cookie until it expires. They sign in again and land in the right half.

This is deliberate. The plugin does not re-run the predicate when reading a session, because roles come from the document rather than from the token — re-checking would buy nothing for security and would log out everyone whose role just changed.

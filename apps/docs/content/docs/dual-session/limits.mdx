...
`@10x-media/impersonation` is the other `@10x-media` plugin that touches auth. It mints sessions with `addSessionToUser` / `jwtSign`, and on isolated targets it writes `generateIsolatedAuthCookie`. List **impersonation after dual-session** (and after any plugin that adds auth collections). A host that assigns `auth.strategies` wholesale after both ran still drops this plugin's strategy.

Third-party plugins that also write cookies or shadow `/login` on the same collection still collide, silently. OAuth, magic links, passkeys, SSO, and 2FA are in that class. Plugins that only read `req.user` or append a strategy compose.

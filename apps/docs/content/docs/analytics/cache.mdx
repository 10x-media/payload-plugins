...
Cache entries are keyed per adapter and query and snapped to the day of the [reporting timezone](/docs/analytics/timezone) (UTC by default). Two reads on the same day therefore share a cached result, and reads in different timezones never collide. Two TTLs apply:

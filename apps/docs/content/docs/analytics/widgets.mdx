...
The plugin registers widgets into Payload's Modular Dashboard (`admin.dashboard.widgets`). Each widget is capability-gated: if no configured adapter can answer its metric or dimension, it is not registered, so dead tiles never appear.
...
Each instance is configurable in the dashboard: an editable title, the metric, a timeframe (the presets plus **Custom range**, which reveals a date picker), and, in multi-adapter setups, a data source. A custom range that exceeds a provider's lookback limit is narrowed to the provider maximum, with a visible note.

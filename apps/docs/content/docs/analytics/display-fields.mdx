...
- `hostname` filters adapter queries to one site in multi-domain setups: a static string, or a resolver with the same signature as `path`. Omit it (or return `null`) to query across every domain the data source records. A value that carries a scheme, a port or capitals is reduced to the bare lowercased host first, so a tenant's `https://Example.com/` domain field still matches what the native engine [stored](/analytics/native#what-counts-as-a-hostname); a value that is no hostname at all reaches the adapter unchanged.
...
`analyticsTab` (and `analyticsTabsField`) renders an interactive panel by default: metric cards with period-over-period deltas (on every source, unless its adapter set `comparison: false` or the previous window starts before the source's `maxLookbackDays`, where the delta is omitted rather than the read failed), a daily trend chart for the first metric, and a timeframe picker that refetches without leaving the document. Reads go through an authenticated `GET /api/analytics/document` endpoint that enforces read access on the document itself, so analytics never leak for content a user cannot see. Pass `interactive: false` to render the static stats row instead.

The endpoint also serves your own UIs: `collection`, `id`, `timeframe` (or `timeframe=custom` with `from`/`to`), `metrics` (comma-separated), `dataSource`, and the `compare=1` / `series=1` flags mirror the panel's reads. `from` and `to` read exactly like the [query endpoint's](/analytics/data-sources): a `YYYY-MM-DD` day is that whole calendar day in the [reporting timezone](/analytics/timezone), `to` included, and a datetime must carry `Z` or an offset. An offset-less datetime, or a `to` before `from`, answers `400 invalid_param` naming the bound at fault in `param`; an inverted window blames `to`.

Its refusals are the plugin-wide [error envelope](/analytics/data-sources#errors), and every answer is `Cache-Control: private, no-store`. Four cases are deliberately one `404 not_found`: a collection with no binding, a missing `id`, a document that does not exist, and a document this caller may not read. Telling them apart would confirm which documents exist to a reader who may not see them. A read that fails for any other reason is a logged `500 internal`, which earlier releases also answered as a `404`; treat a `404` as "nothing to show here" and a `500` as this install being broken.

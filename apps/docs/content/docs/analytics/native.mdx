...
`type`, `path`, and `hostname` are required; anything else is optional. The server derives the rest at ingest time: device class from the user agent, traffic source from the referrer, geo from [resolvers](/docs/analytics/geo), and a salted daily visitor hash (the salt rotates per UTC day, so visitors cannot be tracked across days). The endpoint responds `202` on success and `400` on an invalid body.
...
The engine resolves country, region, and city per event: platform headers first (Vercel, Cloudflare), MaxMind only when headers do not supply a country. Pass `geoDbPath` for MaxMind or `geoResolver` for a custom pipeline. Details on [Geo](/docs/analytics/geo).

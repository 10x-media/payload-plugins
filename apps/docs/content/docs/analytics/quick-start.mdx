...
`type`, `path`, and `hostname` are required; see [Native engine](/analytics/native#the-ingest-endpoint) for the full event shape.
...
- Point the plugin at an existing provider instead: [Adapters](/analytics/adapters).

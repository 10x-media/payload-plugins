...
- A locale the plugin does not ship is added whole.
...
To ship a locale a plugin does not include, translate the keys you care about and pass them for that locale. Payload's `t` has no per-key fallback: a key with no value in the active admin language renders as the key itself, so translate every key you rely on:
...
analytics({
  ...
  translations: {
    ja: {
      [keys.pluginName]: 'アナリティクス',
      [keys.metricPageviews]: 'ページビュー',
    },
  },
})
```

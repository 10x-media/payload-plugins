...
`definePlugin` attaches two pieces of metadata to the returned plugin:

- **`slug`**: the package name, for example `@10x-media/jobs`. Payload builds a slug-keyed map of registered plugins, so plugins can detect each other. Webhooks uses this to switch its delivery mode when Jobs is installed, and to push a trigger into Automations. See [Jobs family interop](/docs/concepts/jobs-family).
- **`order`**: controls when the plugin runs relative to others. Contributors run early (webhooks at `order: 10`), consumers run late (automations at `order: 100`), so contributions are already in place when the consumer reads them.
...
| `./i18n` | Translation `keys`, the built-in `translations`, and the `TranslationsOption` type. See [Translations](/docs/concepts/translations). |

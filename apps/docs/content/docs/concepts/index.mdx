...
- **`slug`**: the package name, for example `@10x-media/jobs`. Payload builds a slug-keyed map of registered plugins, so plugins can detect each other. Webhooks uses this to switch its delivery mode when Jobs is installed, and to push a trigger into Automations. See [Jobs family interop](/concepts/jobs-family).

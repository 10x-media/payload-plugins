...
Several plugins produce background work; one plugin, [Jobs](/jobs), is about running it. They compose automatically through Payload's plugin registry, with zero shared configuration.
...
Detection uses Payload's `definePlugin` slug registry: each plugin registers under its package name, and siblings look it up by slug. See [Plugin conventions](/concepts).

...
The swapped view is referenced by string, so Payload resolves it through the import map. Development regenerates it on boot; a production build uses the committed file. Skip this and the drawer silently falls back to the default list, with `getFromImportMap: PayloadComponent not found` in the server log.
...
The drawer now has **All media** and **By folder** tabs. Double click a folder to enter it, double click a file to pick it.

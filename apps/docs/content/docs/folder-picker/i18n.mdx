...
`@10x-media/folder-picker/i18n` exports `keys` (currently `keys.pluginName`), the built-in English `translations`, and the `TranslationKey` / `TranslationsOption` types. Misspell a key and the build fails.

...
The plugin ships several locales. For a locale it does not ship, Payload renders the raw keys rather than English, so add it through `translations` above rather than waiting for the plugin to grow one.

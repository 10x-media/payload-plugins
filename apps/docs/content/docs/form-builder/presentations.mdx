...
Register through the plugin option to add a presentation to the admin select everywhere, following the [opt-in convention](/docs/concepts#the-opt-in-convention):
...
The plugin option takes descriptors (serializable). To supply the React `Wrapper` an overlay needs, pass a full `FormPresentation` to `<Form>` via its `presentations` prop:

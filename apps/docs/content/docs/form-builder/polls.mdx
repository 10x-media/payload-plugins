...
A poll is a form with its top-level **Poll** checkbox on (`pollEnabled` on `FormDocument` and the forms collection alike; there is no more nested `poll.enabled`). Turning it on reveals a **Poll** tab holding `resultsField`, `type`, `resultsVisibility` (`afterVote` | `afterClose`), and an optional `closesAt`. Once `closesAt` passes, the server rejects new submissions for that form (submission-time guard, not just UI) and `<Poll>` renders a closed notice plus the results. `type` decides how, or whether, the winner gets recorded once the poll closes; see [Outcome](#outcome). A poll with no `closesAt` scheduled can also be closed on demand from the admin; see [Closing manually](#closing-manually).
...
Three types ship, always registered, selected from the poll's `type` field (default `manual`), described as "How the winning option is decided when the poll closes." The select reads `Set the winner manually`, `Most-voted option wins`, and `Winner from the option source`; the last is hidden from the select until at least one poll option source is registered (the strategy itself stays addressable via `poll.types` regardless):
...
A poll with no `closesAt` scheduled shows a **Close poll now** button in the Poll tab (once `closesAt` is set the poll closes on its own, so the button disappears). Clicking it posts to an authenticated `POST /api/forms/:id/close`, which stamps `closesAt` to the current time through the usual `payload.update` (so every closing side effect still runs) and then, for `mostVoted` or `source`, resolves the outcome the same way the scheduled job would. A `manual` poll must already have a winner recorded: closing refuses with an error asking you to set one first, so a poll can never end up closed with no result. Closing an already-closed poll is refused the same way, so the action is safe to click only once it matters.

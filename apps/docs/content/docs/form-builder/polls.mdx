...
That scan-and-replace shape makes `recountPollVotes` unsafe on a `persistSubmissions: false` poll: with submissions pruned, the scan finds nothing to replay, so the delete step wipes the tally down to zero instead of leaving it alone. The tally is the only record of a persist-off poll's votes, so recounting one erases its history rather than healing it; only call it for a poll that keeps its submissions.

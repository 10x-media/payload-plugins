...
`poll.resultsField`, labelled **Vote field** in the admin, is authored as a select listing the form's eligible fields: named instances of field types that declare `pollEligible` (built-in: `select`, `country`, `state`). Server validation enforces the same rule, so a free-text or PII field can never be stored while the poll is enabled; see [Poll lifecycle](#poll-lifecycle) for how enabling a poll binds this field for you.

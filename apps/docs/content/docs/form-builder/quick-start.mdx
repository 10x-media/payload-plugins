...
The title is the document's own, for finding the form in the admin, required in your project's default locale (following your project's localization, if any) so other locales fall back to it. The plugin never renders it: [`<Form>`](/form-builder/rendering) draws the fields and nothing above them, and `form.title` is there for your page to render or ignore.

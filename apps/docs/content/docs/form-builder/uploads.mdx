...
The collection is yours: storage adapter (S3 and friends or `staticDir`), access control, extra fields, admin placement all stay in your hands, including where it sits in the admin nav relative to `forms` and `form-submissions`. The plugin wires its own concerns into it at boot:

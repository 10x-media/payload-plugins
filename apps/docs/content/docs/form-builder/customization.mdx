...
- **Presence-enabled features** are the sanctioned exception: they have no `true`, and supplying the value is the enable. They come in two shapes. Direct `req`-scoped resolvers: `consent.sources`, `email.fromAddresses`, and `email.departments` take a function that resolves options for the current request. Registries of named sources: `email.recipientSources`, `email.fromSources`, and `poll.sources` take a record of entries whose per-entry `resolve` runs when the stored value is used (at send time for the email sources, at render and validation for poll options). `events` follows the same rule with its sink object.

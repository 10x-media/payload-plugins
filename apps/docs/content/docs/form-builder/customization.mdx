...
Every form carries a `buttons` group at the bottom of its Fields tab with three localized labels: `submitLabel`, `nextLabel`, and `backLabel` (next/back apply to multi-step forms). The rendered chrome resolves each label as `<Form>` prop, then the stored value, then the translated default.
...
To assemble the group from scratch instead of receiving the defaults, the individual builders are exported: `buildSubmitLabelField(localize)`, `buildNextLabelField(localize)`, and `buildBackLabelField(localize)`, plus `buildDefaultButtonFields(localize)` returning all three.

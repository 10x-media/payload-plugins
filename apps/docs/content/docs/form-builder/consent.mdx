...
The proof lives on the submission row, so a `persistSubmissions: false` form discards it when the row is [pruned](/form-builder/actions#pruning-submissions) after actions run. That combination is legitimate exactly when the consent record lives elsewhere, a double opt-in provider being the canonical case, and the admin sidebar shows a notice whenever a non-persisting form carries a consent field so nobody has to discover it from the pruning code. A form that needs its own provable consent must keep its submissions.

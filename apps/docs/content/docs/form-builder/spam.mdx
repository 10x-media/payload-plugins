...
`<Form>` renders a visually hidden decoy input (off-screen, `aria-hidden`, out of the tab order, `autoComplete="off"`). Humans never fill it; bots that fill everything do, and the server rejects with a generic error. The decoy field name defaults to `website` (the exported `DEFAULT_HONEYPOT_FIELD`). If you change `spam.honeypot.fieldName` on the server, pass the same name to `<Form honeypot={{ name }}>`, and keep it from colliding with a real field.

...
`text`, `textarea`, `email`, `number`, `date`, `select`, `country`, `state`, `checkbox`, `file`, `consent`, `calculation`, `repeater`, and `message`. All are built through the same `defineFormField` primitive as custom types, so nothing about them is privileged. `file`, `consent`, and `calculation` have their own pages: [Uploads](/form-builder/uploads) (the `file` type only exists once an uploads collection is [configured](/form-builder/uploads#bring-your-own-collection)), [Consent](/form-builder/consent) (likewise, only once [consent sources](/form-builder/consent#setting-it-up) are), [Calculations](/form-builder/calculations).
...
Remove either through the [registry](#the-registry) like any other built-in:

```ts title="payload.config.ts"
formBuilder({
  fields: { country: false, state: false },
})
```

...
Three ship. `emailTeam` emails one or more addresses via `payload.sendEmail`; subject and body are [recall templates](/form-builder/recall-prefill) (`New submission from {{name}}`), and with no email adapter configured it no-ops gracefully. `confirmation` sends to the submitter with the same templating. `signedWebhook` POSTs the submission as JSON with an HMAC signature header (below).

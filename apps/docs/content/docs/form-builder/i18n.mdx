...
- **Content** is what editors author on a form document: labels, placeholders, option labels, consent statements, the response message. It is stored data, localized through Payload's own `localization` on the `forms` collection. Content fields are [localized by default](/form-builder/fields#content-localization); the plugin option `localizeContent: false` opts a project out.

...
Three presets wire [reliability](/docs/jobs/reliability) and [queue control](/docs/jobs/queue-control) consistently for the common deployment shapes. Each returns `{ reliability, queueControl }`, so spread it into the plugin options:
...
`dist/worker.js` is the compiled [worker entrypoint](/docs/jobs/workers).

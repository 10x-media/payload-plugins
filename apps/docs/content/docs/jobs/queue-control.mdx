...
Pass the store to [`createWorker`](/docs/jobs/workers) as `pauseStore` so workers respect it. Pause writes are read-modify-write over kv (last writer wins), which is fine for rare admin actions.
...
| `GET /api/payload-jobs/queue-sweep` | One orphan sweep pass (no leader election; safe from cron). Requires [reliability](/docs/jobs/reliability). |

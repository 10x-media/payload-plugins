...
Picking a task or workflow on the create form fills `input` with a placeholder derived from that entry's `inputSchema`, so the expected shape is on screen instead of in your head. An entry with no schema fills `{}`.

Every task and workflow keeps its own draft while the form is open: switching away parks what you typed, switching back restores it, and a slug you pick for the first time opens on its placeholder. Nothing typed for one task leaks into another. Clearing the selection empties the field. Existing jobs are not touched: their inputs are read-only.

Values come from the field types, and a list carries one element so its shape shows too:
...
The derivation cannot invent meaningful values, only shapes. For a slug whose placeholder should carry real ones, write it yourself; it replaces the derived object whole:

```ts title="payload.config.ts"
jobs({
  input: {
    examples: {
      atosSync: { phases: ['persons'], limit: 100, personCodes: [], runImages: false },
    },
  },
})
```
...
| `input.examples` | `Record<string, Record<string, unknown>>` | `{}` | Hand-written create-form placeholders for `input`, keyed by task or workflow slug, replacing the one derived from `inputSchema`. See [Create form: input placeholder](#create-form-input-placeholder). |

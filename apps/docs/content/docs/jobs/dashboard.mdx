...
Every task and workflow keeps its own draft while the form is open: switching away parks what you typed, switching back restores it, and a slug you pick for the first time opens on its placeholder. Nothing typed for one task leaks into another. A draft that is not valid JSON is not kept. Clearing the selection resets the field to `{}`. Existing jobs are not touched: their inputs are read-only.
...
The derivation cannot invent meaningful values, only shapes. For a slug whose placeholder should carry real ones, write them yourself; they are merged over the derived object, so keys you omit keep their derived value:

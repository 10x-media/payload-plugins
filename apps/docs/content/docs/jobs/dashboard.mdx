...
Every task and workflow keeps its own draft while the form is open: switching away parks what you typed, switching back restores it, and a slug you pick for the first time opens on its placeholder. Nothing typed for one task leaks into another. A draft that is not valid JSON, or an empty object, is not kept: the slug reopens on its placeholder. Clearing the selection resets the field to `{}`. Existing jobs are not touched: their inputs are read-only.

The placeholder is a value, not a hint: what you leave in the editor is what the job receives, `"<users id>"` included.
...
`input.components` swaps the JSON editor for a component of yours, keyed by task or workflow slug. The placeholder above still applies, and the editor is swapped as the selection changes: a slug with an entry gets its editor, every other slug keeps the JSON field.
...
| `placeholder` | `Record<string, unknown> \| undefined` | What the create form would have pre-filled for this slug: the derived object, or its `input.examples` entry. |
| `readOnly` | `boolean \| undefined` | Set on an existing job, whose inputs are locked. |

The editor must be a client component: it writes to the form. It owns no state of its own; every control reads the job's `input` through `useField` and writes a patched object back, which is also how the pre-filled placeholder reaches it:
...
Paths resolve through the import map. They live in plugin options rather than in a field slot the generator walks, so the plugin registers them under `admin.dependencies` for you, and you re-run `payload generate:importmap` after adding or changing one. A path the import map cannot resolve falls back to the JSON field, and Payload logs a console error naming the missing key.

...
1. **Run** (every `runIntervalMs`): claims and runs due jobs, up to `runLimit` per tick, honoring cluster [pause state](/jobs/queue-control) when a `pauseStore` is passed.

...
`start()` runs three guarded loops:

1. **Run** (every `runIntervalMs`): claims and runs due jobs, up to `runLimit` per tick, honoring cluster [pause state](/docs/jobs/queue-control) when a `pauseStore` is passed.
2. **Maintenance** (every `maintenanceIntervalMs`): renews or acquires the `scheduler` and `sweeper` leases and handles schedules while holding `scheduler`.
3. **Sweep** (every `sweepIntervalMs`): recovers orphaned jobs while holding `sweeper`.

Every node runs jobs; only lease holders schedule and sweep. See [Reliability](/docs/jobs/reliability).
...
- **Exec-form CMD.** `CMD ["node", "dist/worker.js"]`, never `CMD node dist/worker.js`. Shell form runs Node under `/bin/sh`, which does not forward SIGTERM, so the worker is hard-killed every time.
- **Grace period.** Set the orchestrator's stop grace to at least `drainTimeoutMs + pollIntervalMs + 5s` (Docker Compose `stop_grace_period`, Kubernetes `terminationGracePeriodSeconds`). Shorter, and the orchestrator SIGKILLs a still-draining worker and you lose the clean requeue.

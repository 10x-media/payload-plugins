...
`jobs({})` alone gives you the [dashboard layer](/jobs/dashboard): a Status column, a queue-health bar, and error/log panels on the `payload-jobs` collection in the admin.
...
Pick a [topology preset](/jobs/topologies) to turn on [reliability](/jobs/reliability) and [queue control](/jobs/queue-control) together:
...
Then run a [worker](/jobs/workers) as its own process, or drive the queue from cron on [serverless](/jobs/topologies#serverless-vercel).

...
<Callout>
A redelivery is a **new** delivery: the fresh row means a fresh `webhook-id`, so a receiver deduplicating on that id treats it as a distinct message rather than a repeat, and will process the payload again. Retries of a *failed* delivery keep the original `webhook-id`, so those do deduplicate. If replaying must not double-process, make your handler idempotent on something inside the body (`data.id`, a version field) rather than on `webhook-id` alone.
</Callout>

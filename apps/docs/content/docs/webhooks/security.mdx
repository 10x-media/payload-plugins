...
- drop admin-managed subscriptions entirely and use only [code subscriptions](/docs/webhooks/subscriptions#code-subscriptions), whose URLs live in your repository and environment.
...
Signing secrets are reveal-once and masked on every read, but stored unencrypted at rest for now. See [Signing](/docs/webhooks/signing). Keep code-subscription secrets in your secret manager, not in the config file.

## Receiver-side checklist

- Verify the signature and enforce a timestamp tolerance ([example](/docs/webhooks/signing#verifying-in-your-receiver)).

...
<Video alt="Hovering a field's help trigger to read the summary, then opening the full guide in a drawer" src="/videos/admin-wiki/field-help.mp4" />
...
One kind of field is skipped: a field that already declares its own `Description` component. The plugin never replaces a consumer component. See [Customization](/admin-wiki/customization#fields-with-a-custom-description) to add the surface back.

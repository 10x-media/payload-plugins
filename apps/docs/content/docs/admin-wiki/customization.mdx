...
A screen the config does not describe, a view registered through `admin.components.views` and anything inside it, is documented with a [custom target](/admin-wiki/targets#custom-targets): declare the key once in the plugin options, then render `WikiCustomHelp` with the bare key.
...
That is the whole integration for a view that wants the plugin's own trigger: the hover card, the guide drawer, and the write affordance, exactly as a field gets them. For a screen that wants its own button or its own placement, see [Building your own surface](#building-your-own-surface).
...
The four config-derived prefixes are stable and documented, so a literal reads fine for those. A custom key is the one to always build through [`customTargetKey`](#building-your-own-surface), never to type with the namespace on it.
...
| `customLabels` | `Record<string, string>` | Declared label per custom target key, in the reader's admin language |
| `blockChips` | `boolean` | Whether the ["Covers" chips](/admin-wiki/wiki-view#the-covers-chips) include blocks |
...
Every surface listens on a [target key](/admin-wiki/targets#target-keys), and the key builders are exported alongside the components, so a key is never spelled by hand:
...
| `TargetChips` | The surfaces a guide covers, as pills, with a `limit` before collapsing to `+N`. Honors `chips.blocks` |
| `WikiWriteGuide` | The write-in-place affordance, in `inline`, `button`, or `menuItem` shape. Takes the target key it should prefill |
| `WikiCustomHelp` | The full help surface for a declared [custom target](/admin-wiki/targets#custom-targets), by bare key |
...
The same shape on a [custom target](/admin-wiki/targets#custom-targets), with your own trigger and your own empty state. Only the key changes:
...
Components you hand the plugin (a [video player](/admin-wiki/authoring#video), a [block renderer](/admin-wiki/authoring#your-own-blocks), a [wiki view slot](/admin-wiki/wiki-view#slots-on-the-index)) are import-map paths. The plugin registers them under `config.admin.dependencies` so `payload generate:importmap` finds them without you referencing them anywhere else.

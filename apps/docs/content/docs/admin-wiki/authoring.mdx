...
**Callout.** Four variants: info, tip, warning, danger. The body is a [nested editor](#the-editor-inside-your-own-block): inline formatting, links, images, video, and the blocks you marked [`nestable`](#your-own-blocks).
...
A block with a `richText` field of its own can run the wiki editor there. `nested: true` drops the callout, headings, lists, blockquotes, horizontal rules, indentation and the fixed toolbar.

Everything else comes from the arguments, so hand it the same object you hand `adminWiki` as `editor`:
  ...
  fields: [
    { name: 'title', type: 'text' },
    {
      ...
      editor: lexicalEditor({
        features: () => wikiFeatures({ ...wikiEditorOptions, nested: true }),
      }),
    },
  ],
}
...
<Callout type="warn">
  Arguments left out fall back to defaults: no blocks of yours, no video, and the stock `wiki-pages` and `wiki-media` slugs. Nothing errors, so an editor missing your blocks, or one pointing at collections you renamed through [`slugs`](/admin-wiki/configuration), looks exactly like one you configured.
</Callout>

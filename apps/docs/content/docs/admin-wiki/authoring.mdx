...
**Callout.** Four variants: info, tip, warning, danger. The body is a nested editor: inline formatting, links, images and video, plus the blocks you marked [`nestable`](#your-own-blocks). It never holds headings or another callout.
...
`nestable: true` also offers the block inside a callout body. Blocks are not nestable by default, inline blocks are:
...
The function form is handed the plugin's features as `defaultFeatures` and returns the whole list, which is the way to reorder or drop one. It runs once per editor, `nested` telling a callout body apart from the guide body:
...
A block with a `richText` field of its own can run the wiki editor there. Pass `nested: true`, which leaves out headings and the callout:
...
export const accordionBlock: Block = {
  ...
  fields: [
    { name: 'title', type: 'text' },
    {
      name: 'body',
      type: 'richText',
      editor: lexicalEditor({ features: () => wikiFeatures({ nested: true }) }),
    },
  ],
}
```
...
<Callout type="warn">
  `wikiFeatures` falls back to the default `wiki-pages` and `wiki-media` slugs. If you renamed them through [`slugs`](/admin-wiki/configuration), pass the same values as `pagesSlug` and `mediaSlug`, or guide links and uploads inside your block point at collections that do not exist.
</Callout>

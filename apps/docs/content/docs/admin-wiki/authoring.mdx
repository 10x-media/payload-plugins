...
A block sits between paragraphs. For one that sits inside a sentence, a chip or a status badge, use `inlineBlocks`:
...
An inline block never appears in the block insert menu, and its renderer has to return inline-level JSX.
...
A feature puts a node in the document; a converter decides what that node renders as in a published guide. Blocks and inline blocks already carry theirs as the `component` each declares, so reach for this when a feature contributes a node type of its own.

Point the plugin at a client module exporting a `WikiConvertersFunction`, the same shape as the `converters` prop on Payload's [`RichText`](https://payloadcms.com/docs/rich-text/converting-jsx):
...
The module resolves through the import map exactly as a block renderer does, and must be a client module.

Converters apply wherever the plugin renders a guide: the wiki view, the surface drawers, and a node nested inside a callout body. Rendering `GuideArticle` yourself, pass a `converters` function as a prop and it applies over the project's.

...
<Video alt="Turning on edit mode, then opening a create drawer from an undocumented field with its target prefilled" src="/videos/admin-wiki/write-in-place.mp4" />
...
[Excluded](/admin-wiki/configuration#exclusions) entities carry no surfaces, so there is nowhere for an affordance to render and no way to reach a create drawer prefilled with one.

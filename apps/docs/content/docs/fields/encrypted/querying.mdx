...
`unique` is a field property (first argument); `queryable` is an encryption option (second argument). Blind indexing is available only for `text`, `email`, and `number` fields, the scalar types the index can key on. It is rejected on [`protection: 'writeOnly'`](/fields/encrypted/write-only) fields: equality lookup on a never-readable secret is a guessing oracle.
...
- **Exact match only.** No `like`, `contains`, comparison, or sort; the index is a fingerprint, not an order-preserving value. See [Supported operators](#supported-operators) for the full set and how unsupported operators behave.

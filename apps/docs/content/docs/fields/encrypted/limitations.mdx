...
- No `like`, `contains`, comparison operators, or sorting on encrypted fields. The database sees opaque strings.
- Exact-match `equals` / `in` and `unique` work only with [`queryable: true`](/fields/encrypted/querying), which is available for `text`, `email`, and `number` fields, and inherits the blind-index caveats below.
- Admin list-view search and filters that rely on substring matching skip encrypted fields.
...
- **`richText`**: the Lexical JSON encrypts as one blob; sub-field queries and any server-side traversal of the content tree are lost. It is masked-only in the admin (the editor cannot run on an encrypted backing field), so the edit view shows read-only JSON and content is edited through the API.

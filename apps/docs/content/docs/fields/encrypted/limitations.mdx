...
The binding's first component is the slug by default, which makes renaming a collection whose data stays in place a re-keying event: every stored value fails authentication under the new name. A field whose collection slug is configurable (plugin-owned collections) should pin the component instead; see [aadScope](/fields/encrypted/key-management#pinning-the-aad-scope).

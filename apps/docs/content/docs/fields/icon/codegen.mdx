...
A name must be non-empty and free of whitespace, control characters, `:` (which separates library from name in a stored value), and the `'` and `\` that would escape the emitted import key. Everything else is legitimate, including uppercase, digits, dots and non-latin scripts, so a library keyed by code (`HUN`, `SUI`) generates like any other.

Names that differ only by case are rejected. Drawer search normalises case, so `HUN` beside `hun` would be indistinguishable to an editor.

...
`fallback` renders when the value is empty, malformed, or names an icon the registered adapters cannot resolve (for example a library you stopped registering). Without a fallback the component renders nothing; it never throws on bad data. Both factories honour it identically.
...
A renderer adapter may take `layers` for a library whose glyphs come from more than one place, such as a package plus overrides. Each is asked in turn, later first, so an override wins and an older layer still serves names the newer one lacks. See [layers](/fields/icon/layers).

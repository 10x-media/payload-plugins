---
title: Fields
description: Reusable Payload v3 fields that look and behave native. Color picker, icon picker, encrypted fields, unit-aware measurement fields, and a phone number field in one package.
---
...
<Cards>
  ...
  <Card title="Measurement" href="/fields/measurement" description="A number field storing one canonical unit while each viewer edits and reads in their own, with locale detection and compound units like feet-and-inches." />
  <Card title="Phone number" href="/fields/phone" description="A country-aware phone number field with a searchable picker, live formatting, and validation and metadata sized to the install." />
</Cards>

## One package, isolated bundles

Every family lives behind its own subpath export (`/color`, `/icon`, `/encrypted`, `/measurement`, `/phone`, plus `/color/utils`, `/icon/react`, `/icon/codegen`, `/measurement/utils`, `/phone/utils`). Importing one family never pulls another into your bundle, icon datasets load lazily and only in the admin, and the encrypted machinery is server-only code that never enters a client bundle. These are tested invariants, enforced on the built output in CI.

...
Every admin-facing string, the view title, the filter labels, the operation badges, the empty state, is a typed translation under the `auditLogs:` namespace. Several locales ship built in; the `translations` export names the current set.

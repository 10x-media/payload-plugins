...
| `auth` | `false \| Record<CollectionSlug, false \| { login?, forgotPassword? }>` | all on | Auth event logging. On for every auth collection unless told otherwise. |
...
| `drafts` | `'ignore' \| 'log'` | `'log'` | Default draft handling for versioned entities. |
...
| `drafts` | `'ignore' \| 'log'` | inherits | `'ignore'` skips draft saves and diffs a publish against the last published version. |
